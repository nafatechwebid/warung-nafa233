...
async function loginAdmin() {
  ...
  try {
    ...
    errBox.textContent = "Terjadi error teknis: " + (e && e.message ? e.message : e);
    console.error("Error saat login admin:", e);
  }
}
...
async function updateStokCepat(id, val) {
  await supabase.from("produk").update({ stok: parseInt(val) || 0 }).eq("id", id);
}
...
async function muatPesananAdmin() {
  ...
  supabase.channel("pesanan-admin")
...
async function setujuiPembayaranTunai(id) {
  ...
  await supabase.from("pesanan").update({ metode_bayar: "tunai", status: "dibayar" }).eq("id", id);
}
...
async function simpanQris() {
  ...
  const { error } = await supabase.from("pengaturan").update({ qris_url: qrisBase64Sementara }).eq("id", "toko");
...
async function simpanRekening() {
  ...
  const { error } = await supabase.from("pengaturan").update(data).eq("id", "toko");
...
async function muatUlasanAdmin() {
  const muat = async () => {
    const { data } = await supabase.from("ulasan").select("*").order("created_at", { ascending: false });
    ulasanList = data || [];
    renderUlasanAdmin();
  };
  await muat();

  supabase.channel("ulasan-admin")
...
async function bukaChatAdmin(idPesanan, namaPelanggan) {
  ...
  if (chatChannelAdmin) supabase.removeChannel(chatChannelAdmin);
  chatChannelAdmin = supabase.channel("chat-admin-" + idPesanan)
...
function tutupChatAdmin() {
  ...
  if (chatChannelAdmin) { supabase.removeChannel(chatChannelAdmin); chatChannelAdmin = null; }
  idPesananChatAdminAktif = null;
}

// ---------- PWA ----------
// SEMENTARA DINONAKTIFKAN untuk troubleshooting login.
// Fungsi ini sekarang MEMBATALKAN semua service worker yang mungkin
// masih aktif dari percobaan sebelumnya (bukan mendaftarkan yang baru),
// supaya tidak ada lagi file/cache lama yang nyangkut.
// Setelah login sudah pasti berhasil & stabil, baru kita aktifkan lagi
// pendaftaran service worker yang sebenarnya.
