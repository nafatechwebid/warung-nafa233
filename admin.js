...
function masukAdmin() {
  ...
  if (localStorage.getItem("nafa233_notif_aktif") === "1") {
...
async function muatPesananAdmin() {
  ...
  sb.channel("pesanan-admin")
    .on("postgres_changes", { event: "INSERT", schema: "public", table: "pesanan" }, payload => {
      notifikasiPesananBaru(payload.new);
      muat();
    })
    .on("postgres_changes", { event: "UPDATE", schema: "public", table: "pesanan" }, muat)
    .subscribe();
}

// ---------- NOTIFIKASI PESANAN BARU ----------
function aktifkanNotifikasi() {
  const audio = document.getElementById("bunyiPesananBaru");
  // Mainkan sekali (senyap sekilas) supaya browser "izinkan" audio ini
  // diputar otomatis nanti tanpa perlu sentuhan pengguna lagi.
  audio.volume = 0;
  audio.play().then(() => {
    audio.pause();
    audio.currentTime = 0;
  ...
  if ("Notification" in window) {
    Notification.requestPermission().then(izin => {
      localStorage.setItem("nafa233_notif_izin", izin === "granted" ? "1" : "0");
    });
  }

  localStorage.setItem("nafa233_notif_aktif", "1");
  document.getElementById("btnAktifkanNotif").textContent = "🔔 Notifikasi Aktif";
  document.getElementById("btnAktifkanNotif").disabled = true;
}

function notifikasiPesananBaru(pesananBaru) {
  if (localStorage.getItem("nafa233_notif_aktif") !== "1") return;
  ...
  if ("Notification" in window && Notification.permission === "granted") {
    const notif = new Notification("🧾 Pesanan Baru - Warung Nafa233", {
      body: `${pesananBaru.nama_pelanggan} · ${formatRupiah(pesananBaru.total)} · Jam ambil ${pesananBaru.jam_ambil}`,
      icon: "icon-192.png",
      tag: "pesanan-" + pesananBaru.id
    });
...
function renderUlasanAdmin() {
  ...
  wrap.innerHTML = ulasanList.map(u => `
    <div class="admin-ulasan-row">
      <strong>${u.nama_pelanggan}</strong> — <span class="bintang-tampil">${"★".repeat(u.rating)}</span><span class="bintang-kosong">${"☆".repeat(5 - u.rating)}</span>
...
async function kirimChatAdmin() {
  ...
  input.value = "";
  if (chatMuatAdminAktif) chatMuatAdminAktif();
}
