...
// ---------- KATALOG ----------
async function ambilProdukDariDb() {
...
function muatProduk() {
  ambilProdukDariDb();
  sb.channel("produk-toko")
    .on("postgres_changes", { event: "*", schema: "public", table: "produk" }, ambilProdukDariDb)
    .subscribe();
}
...
function gantiTab(tab) {
  ...
  if (intervalCekUlasan) { clearInterval(intervalCekUlasan); intervalCekUlasan = null; }
  tabAktif = tab;
  suhuAktif = "semua";
...
async function buatPesanan() {
  ...
  const { data, error } = await sb.rpc("buat_pesanan", {
  ...
  if (metode === "tunai") {
    ...
    document.getElementById("lokasiTunaiBox").classList.add("hidden");
  }
}
...
function mulaiTimerTunai(idPesanan) {
  ...
  const linkLokasi = document.getElementById("linkLokasiWarung");
  linkLokasi.href = LOKASI_WARUNG_URL;
  document.getElementById("lokasiTunaiBox").classList.remove("hidden");
  ...
  timerInterval = setInterval(async () => {
    ...
    if (sisaDetik <= 0) {
      ...
      await sb.rpc("alihkan_ke_qris", { p_pesanan_id: idPesanan });
      document.getElementById("timerTunaiBox").classList.add("hidden");
      document.getElementById("lokasiTunaiBox").classList.add("hidden");
...
// ---------- PESANAN SAYA & CHAT ----------
let ulasanTerkirimSet = new Set();
let ratingPerPesanan = {};
let pesananSayaTerakhir = [];
let intervalCekUlasan = null;
...
async function bukaPesananSaya() {
  ...
  const muat = async () => {
    ...
    pesananSayaTerakhir = data || [];

    const idList = pesananSayaTerakhir.map(p => p.id);
    if (idList.length > 0) {
      const { data: ulasanData } = await sb.from("ulasan").select("pesanan_id").in("pesanan_id", idList);
      ulasanTerkirimSet = new Set((ulasanData || []).map(u => u.pesanan_id));
    }
    renderPesananSaya();
  };
  await muat();

  sb.channel("pesanan-saya")
    .on("postgres_changes", { event: "*", schema: "public", table: "pesanan", filter: `wa_pelanggan=eq.${sesi.wa}` }, muat)
    .subscribe();

  // Cek berkala supaya form ulasan otomatis muncul begitu genap 5 menit,
  // tanpa perlu pelanggan refresh halaman.
  if (intervalCekUlasan) clearInterval(intervalCekUlasan);
  intervalCekUlasan = setInterval(renderPesananSaya, 15000);
}

function bolehUlasan(p) {
  if (p.status !== "selesai" || !p.selesai_pada) return false;
  if (ulasanTerkirimSet.has(p.id)) return false;
  const limaMenitLalu = Date.now() - 5 * 60 * 1000;
  return new Date(p.selesai_pada).getTime() <= limaMenitLalu;
}

function renderPesananSaya() {
  const wrap = document.getElementById("listPesananSaya");
  const list = pesananSayaTerakhir;
  if (list.length === 0) {
  ...
  wrap.innerHTML = list.map(p => `
    <div class="pesanan-saya-card">
      ...
      ${ulasanTerkirimSet.has(p.id) ? `<p class="muted">✅ Terima kasih, ulasan untuk pesanan ini sudah dikirim.</p>` : ""}
      ${bolehUlasan(p) ? renderFormUlasan(p) : ""}
    </div>
  `).join("");
}

function renderFormUlasan(p) {
  const rating = ratingPerPesanan[p.id] || 0;
  const bintang = [1, 2, 3, 4, 5].map(n => `
    <span class="${n <= rating ? "aktif" : ""}" onclick="pilihBintang('${p.id}', ${n})">★</span>
  `).join("");

  return `
    <div class="ulasan-inline">
      <p>Pesanan dibayar dengan ${labelMetodeBayar(p.metode_bayar)} sebesar ${formatRupiah(p.total)}.</p>
      <p><strong>⭐ Bagaimana Pelayanannya?</strong></p>
      <div class="bintang-input" id="bintang-${p.id}">${bintang}</div>
      <textarea id="komentar-${p.id}" placeholder="Tulis komentar/ulasan kamu..."></textarea>
      <button onclick="kirimUlasan('${p.id}')">Kirim Ulasan</button>
    </div>
  `;
}

function pilihBintang(idPesanan, nilai) {
  ratingPerPesanan[idPesanan] = nilai;
  renderPesananSaya();
}

async function kirimUlasan(idPesanan) {
  const sesi = ambilSesiPelanggan();
  const rating = ratingPerPesanan[idPesanan] || 0;
  const komentarEl = document.getElementById("komentar-" + idPesanan);
  const komentar = komentarEl ? komentarEl.value.trim() : "";

  if (rating === 0) { alert("Pilih bintang dulu ya."); return; }
  if (!sesi) { alert("Silakan login dulu untuk memberi ulasan."); return; }

  const { error } = await sb.from("ulasan").insert({
  ...
  if (!error) {
    ulasanTerkirimSet.add(idPesanan);
    delete ratingPerPesanan[idPesanan];
    renderPesananSaya();
  } else {
...
async function bukaChat(idPesanan) {
  ...
  chatChannel = sb.channel("chat-" + idPesanan)
...
async function kirimChatPelanggan() {
  ...
  input.value = "";
  if (chatMuatAktif) chatMuatAktif();
}
